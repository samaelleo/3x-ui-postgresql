...
func (s *ServerService) GetDb() ([]byte, error) {
	if config.GetDBType() == "postgres" {
		return nil, common.NewError("Database backup via panel is only supported for SQLite.")
	}
...
func (s *ServerService) ImportDB(file multipart.File) error {
	if config.GetDBType() == "postgres" {
		return common.NewError("Database restore via panel is only supported for SQLite.")
	}
